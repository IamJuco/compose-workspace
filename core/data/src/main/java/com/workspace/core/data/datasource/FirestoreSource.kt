...
interface FirestoreSource {
    ...
    suspend fun getUserInfo(userId: String): ServiceResult<UserDocument>
}
...
class FirestoreSourceImpl @Inject constructor(
    ...
    override suspend fun getUserInfo(userId: String): ServiceResult<UserDocument> {
        return try {
            val documentSnapshot = firestore.collection("users")
                .document(userId)
                .get()
                .await()
            val user = documentSnapshot.toObject(UserDocument::class.java)
            if (user != null) {
